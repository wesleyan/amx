...
(***********************************************************)
(***********************************************************
 !!FILE REVISION: JuddB6main
    REVISION DATE: 11/Feb/2008
    COMMENTS: Due to projector issues with RGB1, RGB2 is
	used instead where it used to be RGB1. Therefore,
	following array was altered.
    'VOLATILE INTEGER SUBNAV_INPUT_SIGNAL[]'
    Replace all instances of Proj_RGB2_in 
    with proj_RGB1_in when projector is swapped 
    out to revert to old standard.	                   *)
(***********************************************************)
(***********************************************************)
(*!!FILE REVISION: JuddB6main                              *)
(*	REVISION DATE: 11/05/2007			   *)
(*	COMMENTS: Netlinx was changed to NI-3000	   *)
(*		Code found was 6 rs232 port NI		   *)
(***********************************************************)
(*!!FILE REVISION: Rev 0                                   *)
...
dvRELAY         = 5001:8:0       // 3 SCREENS RELAYS 1-6 MOMENTARY 'SCREEN2' NOT IN ALL SYSTEMS

dvVCR           = 5001:9:0       // VCR 

dvDVD           = 5001:10:0       // DVD PLAYER
...
//Removed for testing, i!-timemanagermod.tko required
(***********************************************************)
...
DEFINE_CALL 'SETUP' (CHAR SYSTEM[])
{
	SWITCH(SYSTEM)
	{
      ...
      VSWITCH_LOOKUP[RACK_MAC]    = 2
		...
		VSWITCH_LOOKUP[DOC_CAM]     = 0       
		VSWITCH_LOOKUP[RACK_VID]    = 5
		VSWITCH_LOOKUP[FB_VID]      = 0      // PODIUM VID
		}
	} 
}
(***********************************************************)
...
DEFINE_MODULE 'ExtronSystem10VideoSwitchMod' ex1(dvROUTER,vdvROUTER)
...
DEFINE_MODULE 'NEC1065_f' ptest2(dvProj2,vdvProj2)
...
BUTTON_EVENT[dcSource] (* GET_LAST = 1 - 15 *)
{
  PUSH:
  {
    ...
    //Debug test code
    //Send_string 0, "'Henry:Get_last(dcsource) = ',ITOA(GET_LAST(dcSource))"
    //Send_string 0, "'Henry:SOURCE = ',ITOA(SOURCE)"
    //Debug test code end
