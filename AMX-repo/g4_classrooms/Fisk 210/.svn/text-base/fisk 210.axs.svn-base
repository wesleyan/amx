...
INTEGER EXTRA_BUTTONS[][] = {
    {5, 6},
    {SOURCE_LAPTOP_PODIUM, SOURCE_LAPTOP_RACK},
    {0},
    {0},
    {SOURCE_AUX_PODIUM, SOURCE_AUX_RACK}
}
...
DEFINE_MODULE 'DVDIRModule' dvdModule(dvDVDPlayer, vdvDVDPlayer, transport_inputs, dvd_menu_nav)
...
DATA_EVENT[vdvRoomSpecific]
{
	...
	SELECT
	{
		...
		{
		    CALL 'switch source'(ATOI(LEFT_STRING(DATA.TEXT, FIND_STRING(DATA.TEXT,',',1)-1)), 1)
		}
	    }
...
{
    PUSH: {
	PULSE[vdvDVDPlayer, dvd_menu_nav[GET_LAST(dvd_menu_nav)]]
    }
}
