...
VOLATILE INTEGER Proxy_FOCUSFAR = 20
VOLATILE INTEGER Proxy_FOCUSNEAR = 21
VOLATILE INTEGER Proxy_FOCUSFARFINEPRESS = 22
VOLATILE INTEGER Proxy_FOCUSNEARFINEPRESS = 23
VOLATILE INTEGER Proxy_FOCUSFARFINEHOLD = 26
VOLATILE INTEGER Proxy_FOCUSNEARFINEHOLD = 27
VOLATILE INTEGER Proxy_FOCUSFINERELEASE = 28
VOLATILE INTEGER Proxy_AUTOFOCUS = 24
VOLATILE INTEGER Proxy_FOCUSSTOP = 25
...
VOLATILE INTEGER ACTION_group[]=
{
  ...
  Proxy_FOCUSFAR,
  Proxy_FOCUSNEAR,
  Proxy_FOCUSFARFINEPRESS,
  Proxy_FOCUSNEARFINEPRESS,
  Proxy_AUTOFOCUS,
  Proxy_FOCUSSTOP,
  Proxy_FOCUSNEARFINEHOLD,
  Proxy_FOCUSFARFINEHOLD,
  Proxy_FOCUSFINERELEASE
}
...
VOLATILE CHAR Real_Remote_FocusFar[] = 'FCONTDRV=FAR'
VOLATILE CHAR Real_Remote_FocusNear[] = 'FCONTDRV=NEAR'
VOLATILE CHAR Real_Remote_FocusFarFine[] = 'FSTEPDRV=FAR'
VOLATILE CHAR Real_Remote_FocusNearFine[] = 'FSTEPDRV=NEAR'
VOLATILE CHAR Real_Remote_AutoFocus[] = 'AUTOSETEXE FOCUS'
VOLATILE CHAR Real_Remote_FocusStop[] = 'FCONTDRV=STOP'


VOLATILE CHAR Real_Remote_CmdsList[12][17]=
{
  'POWER ON',              //on
  'POWER OFF',             //off
  'BLANK=ON',              //mute on
  'BLANK=OFF',             //mute off
  'FCONTDRV=FAR',          //focus far coarse
  'FCONTDRV=NEAR',         //focus near coarse
  'FSTEPDRV=FAR',          //focus far fine
  'FSTEPDRV=NEAR',         //focus near fine
  'AUTOSETEXE FOCUS',      //Auto focus
  'FCONTDRV=STOP',
  'FSTEPDRV=FAR',
  'FSTEPDRV=NEAR'
}
...
VOLATILE CHAR RealRemote_CmdsList[][]=
{
  ...
  Real_Remote_FocusFar,             //focus farther
  Real_remote_FocusNear,            //focus nearer
  Real_remote_AutoFocus,            //auto focus
  Real_remote_FocusStop             //stop the focus motor
}
...
integer ready_for_next_focus_command = true
INTEGER sentStop = false
INTEGER iterator = 0
...
DEFINE_FUNCTION INTEGER  switchinputs(INTEGER sourceNum)
{
    ...
    // 5 - DIG	6 - DIV
    // 7 - COMP	8 - SCART
  SEND_STRING 0, 'Is this getting run for some reason?'
  IF( sourceNum <= 8 )
  {
...
{ 
  ONLINE:
  {
    ...
    //SEND_STRING devicestrings, "Real_Mode_REMOTE,$00"
  ...
  { 
	...
	SWITCH(ProjBuff[1])
	{
		...
		if(!ready_for_next_focus_command)
		{
		    ready_for_next_focus_command = true
		    SEND_STRING devicestrings, 'OK_TO_SEND'
		}
...
CHANNEL_EVENT[vdvProxy,ACTION_group]
{
	ON:
	{
	    if(projSettings.MODE <> 'REMOTE')
	    {
		SEND_STRING devicestrings, "Real_Mode_REMOTE"
		SEND_STRING devicestrings, 'GET MODE';
	    }
    ...
    //		[vdvProxy,Proxy_FB_MUTE] = ![vdvProxy, Proxy_FB_MUTE]
		...
		CASE Proxy_FOCUSNEAR:
		CASE Proxy_FOCUSFAR:
		{
		    //SEND_STRING devicestrings, 'NOT_OK_TO_SEND'
		    /*TIMED_WAIT_UNTIL (projSettings.MODE == 'REMOTE') 50 //wait up to five seconds for us to be in remote mode
		    {
			ready_for_next_focus_command = false
			
			SEND_STRING 5001:6:0,"Real_Remote_CmdsList[GET_LAST(ACTION_group)],$00"
		    }
		    WAIT 50 //make sure we don't leave it out of OK_TO_SEND mode for too long
		    {
			SEND_STRING devicestrings, 'OK_TO_SEND'
			//SEND_STRING devicestrings, "Real_Mode_LOCAL"
		    }*/
		    SEND_STRING devicestrings, "Real_Remote_CmdsList[GET_LAST(ACTION_group)]"
		    SEND_STRING devicestrings, 'FCONTDRV=STOP'
		    SEND_STRING devicestrings, "Real_Mode_LOCAL"
		    SEND_STRING devicestrings, 'GET MODE'
		}
		CASE Proxy_FOCUSNEARFINEPRESS:
		CASE Proxy_FOCUSFARFINEPRESS:
		{
		    SEND_STRING devicestrings, 'NOT_OK_TO_SEND'
		    TIMED_WAIT_UNTIL (projSettings.MODE == 'REMOTE') 50 //wait up to five seconds for us to be in remote mode
		    {
		...
		CASE Proxy_FOCUSNEARFINEHOLD:
		CASE Proxy_FOCUSFARFINEHOLD:
		{
		    SEND_STRING devicestrings, 'NOT_OK_TO_SEND'
		    TIMED_WAIT_UNTIL (projSettings.MODE == 'REMOTE') 50 //wait up to five seconds for us to be in remote mode
		    {
		...
		CASE Proxy_FOCUSFINERELEASE:
		{
		    SEND_STRING devicestrings, 'OK_TO_SEND'
		    SEND_STRING devicestrings, "Real_Mode_LOCAL"
		    SEND_STRING devicestrings, 'GET MODE'
		}
		CASE Proxy_FOCUSSTOP:
		{
		    sentStop = false
		    for(iterator = 0; iterator < 10; iterator++) //here we just blast stop commands in the hope that one will get through
		    {
			WAIT 1 
			{
			    SEND_STRING 5001:6:0, "'FCONTDRV=STOP', $00"
			}
		    }
		    TIMED_WAIT_UNTIL (ready_for_next_focus_command==true) 5 //cancel the wait after 0.5 seconds
		    {
			sentStop = true
			SEND_STRING 5001:6:0, "'FCONTDRV=STOP',$00"
			SEND_STRING devicestrings, 'OK_TO_SEND'
