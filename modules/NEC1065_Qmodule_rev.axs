MODULE_NAME='NEC1065_Qmodule_rev' (DEV dvreal , DEV vdvProxy, CHAR xmlout[50000])
...
{(*checksum is the last byte of a data frame, 
  it is equal the LOWER 8 BITS of the sum of the rest of the frame *)
    ...
    IF (length < 6) RETURN 0  //return FALSE if only header and cks found (no data)
    
    chksum = in1[length] //SET chksum
    
    FOR(x=1; x < length ; x++) frmsum = frmsum + in1[x] //calc frmsum
    
    IF( (frmsum BAND $FF) == chksum) RETURN length //return TRUE if lower 8 bits of frmsum == checksum
    ELSE RETURN 0
}
...
DEFINE_FUNCTION CHAR[300] PRINT_HEX(CHAR TSTRING[100])
{
	...
	IF ((HOLDER >= 65 AND HOLDER <=90)||(HOLDER >= 97 AND HOLDER <=122)) RETURN_STR = "RETURN_STR,HOLDER,','"
	ELSE RETURN_STR = "RETURN_STR,ITOHEX(HOLDER),','"
    }
...
IF( var1 <= 7 )
{
  ...
  //SEND_STRING dvreal,"str_output,calc_checksum(str_output)"
	...
	SEND_STRING devicestrings, "str_output,calc_checksum(str_output)"
...
{
	runsense.proj_status        = in1 & $02         //PROJECTOR STATUS
	[vdvproxy,ON_fb] = in1 & $02
	
	runsense.ex_control         = in1 & $10         //EXTERNAL_CONTROL
	[vdvproxy,127] = in1 & $10
	
	runsense.cooling_processing = in1 & $20         //PROJECTOR COOLING
	[vdvproxy,COOLING_fb] = in1 & $20 
	
	runsense.selecting_signal   = in1 & $40         //INPUT CHANGING
	[vdvproxy,126] = in1 & $40
	
	runsense.power_processing   = in1 & $80         //POWER PROCESSING !!!!!!!!!!!!CHANGE!!!!!!!!!!!!
	[vdvproxy,114] = in1 & $80
}
...
{ 
  ...
  { 
	...
	/* I have no idea what this is doing or why it's failing at the moment. -Sam DeFabbia-Kane
...
CHANNEL_EVENT[vdvProxy,ACTION_group]
{
	ON:
	{
		//SEND_STRING 0, "'Sending command: ', itoa(ACTION_hex_cmds[GET_LAST(ACTION_group)])"
		...
		SEND_STRING devicestrings, "ACTION_hex_cmds[GET_LAST(ACTION_group)]"
