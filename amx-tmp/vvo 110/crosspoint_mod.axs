MODULE_NAME='crosspoint_mod' (DEV dvreal, DEV vdvproxy, INTEGER switcher_ties[50])
...
{
    //command_queue[LENGTH_ARRAY(command_queue)+1] = "ITOA(input),'*',ITOA(output),'!'"
    //SET_LENGTH_ARRAY(command_queue,LENGTH_ARRAY(command_queue)+1)
    //send_string dvreal,"ITOA(input),'*',ITOA(output),'!'"
	SWITCH (tietype)
	{
	  CASE tie_all:
	  {
