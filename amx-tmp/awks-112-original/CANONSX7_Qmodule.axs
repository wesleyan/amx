...
VOLATILE CHAR Real_Remote_RGB1[] =  'RC INPUT=A-RGB1'
VOLATILE CHAR Real_Remote_RGB2[] =  'RC INPUT=A-RGB2'
VOLATILE CHAR Real_Remote_VID[] =   'RC INPUT=VIDEO'
VOLATILE CHAR Real_Remote_SVID[] =  'RC INPUT=S-VIDEO'
VOLATILE CHAR Real_Remote_DIG[] =   'RC INPUT=D-RGB'
VOLATILE CHAR Real_Remote_DIV[] =   'RC INPUT=D-VIDEO'
VOLATILE CHAR Real_Remote_COMP[] =  'RC INPUT=COMP'
VOLATILE CHAR Real_Remote_SCART[] = 'RC INPUT=SCARTRGB'

VOLATILE CHAR Real_RemoteChanList[8][18] =
{
 'RC INPUT=A-RGB1',
 'RC INPUT=A-RGB2',
 'RC INPUT=VIDEO',
...
(********** Projector status flat ************)
VOLATILE INTEGER PROJECTOR_WARMED_UP = 0
...
DEFINE_CALL 'parse_GET_POWER'(CHAR projBuff[]) {
	...
	PROJECTOR_WARMED_UP = 0
    } ELSE IF (isEqual(projSettings.POWER, 'ON', 1, 1)) {
	ON[vdvProxy, Proxy_FB_ON]
	OFF[vdvProxy,Proxy_FB_COOLING]
	//Corrects for projector issue where the first video mute call doesn't work
	if(PROJECTOR_WARMED_UP == 0)
	{
	    SEND_STRING devicestrings, "Real_Remote_CmdsList[3]"
	    SEND_STRING devicestrings, "Real_Remote_CmdsList[4]"
	    PROJECTOR_WARMED_UP = 1
	}
...
CHANNEL_EVENT[vdvProxy,ACTION_group]
{
	...
	SWITCH(CHANNEL.CHANNEL) {
	    CASE Proxy_MUTEON: {
		SEND_STRING devicestrings, "'GET BLANK'"
