...
(***********************************************************)
/* Renaming to DAC100 Main Code.
   Minor modifications to channel names.
   Changed projector module for dvproj to CanonSX7
...
VOLATILE LONG TL_WARMING			=	9


VOLATILE INTEGER TRUE = 1
VOLATILE INTEGER FALSE = 0
...
PROJ_MUTE_OFF		= 6
*/
//Not used in Canon module, here for code preservation
PROJ_LAMP_RESET = 66
PROJ_WARMING_FB = 3


//Same channels, new names
Proj_ON_IN = 1
Proj_OFF_IN = 4
Proj_MUTEON_IN = 5
Proj_MUTEOFF_IN = 6
...
//New error feedback channels
Proj_ERR_NONE = 201
Proj_ERR_TEMP = 202
Proj_ERR_LAMP = 203
Proj_ERR_LAMP_COVER = 204
Proj_ERR_COOLING_FAN = 205
Proj_ERR_PWR_SUPPLY = 206
Proj_ERR_AK = 207
Proj_ERR_ASC = 208
Proj_ERR_AF = 209
Proj_ERR_POWER_ZOOM = 210
Proj_ERR_POWER_FOCUS = 211
...
DEVCHAN device_commactive[]={
	{vdvROUTER,255},
	{vdvPCSWITCH,255}
}

VOLATILE CHAR device_commactive_txt[2][20]=
{
	'Extron',
	'Blackbox KVM'
}	
...
{
	PROJ_RGB2_IN,	//'RACK PC',      // 1
	PROJ_DVI_IN,	//'RACK MAC',      // 2
	PROJ_RGB2_IN,	//'RACK LAPTOP',      // 3
	PROJ_RGB2_IN,	//'FLOOR BOX LAPTOP',      // 4
	PROJ_RGB2_IN,	//'WALL BOX LAPTOP',      // 5
	PROJ_RGB2_IN,	//'PODIUM LAPTOP',      // 6
	PROJ_RGB2_IN,	//'DOCUMENT CAMERA',      // 7
	PROJ_VID_IN,	//'VCR',      // 8
	PROJ_VID_IN,	//'DVD',      // 9
...
DEFINE_CALL 'SETUP' (CHAR SYSTEM[])
{
	SWITCH(SYSTEM)
	{
		...
		VSWITCH_LOOKUP[FB_LAP]      = 11
		VSWITCH_LOOKUP[WB_LAP]      = 8
		...
		VSWITCH_LOOKUP[POD_LAP]     = 5               // CHANGED DUE TO CONDUIT ISSUE
		VSWITCH_LOOKUP[DOC_CAM]     = 0               // HAD TO MAKE WORK WITH WHAT 
		VSWITCH_LOOKUP[VCR]         = 10               // THEY HAD
		VSWITCH_LOOKUP[DVD]         = 3               
		VSWITCH_LOOKUP[RACK_VID]    = 6               
		VSWITCH_LOOKUP[FB_VID]      = 7
		VSWITCH_LOOKUP[VCR_CC]      = 10 		
		}
		CASE 'SC 509A':
		{
		VSWITCH_LOOKUP[RACK_PC]     = 1
...
DEFINE_MODULE 'CANONSX7_Qmodule' ptest1(dvProj,vdvProj, ProjectorXML)
...
CHANNEL_EVENT[device_commactive]
{
	OFF:
	{
	SEND_STRING 0, 'Email sent!'
			SmtpQueMessage('amx@wesleyan.edu',
								 EmailToWho,
								 "'AMX Alert - ',SystemName[SYSTEM_NUMBER]",
								 "'A device has been powered off or has lost connectivity.',13,10,
								 'Room: ',SystemName[SYSTEM_NUMBER],13,10,
								 'Device: ',device_commactive_txt[GET_LAST(device_commactive)],13,10,
								 'Time: ',TIME,13,10",
								 '')
	}
}
...
BUTTON_EVENT[vdvTP,91]                  // EXIT YES BUTTON
{
  PUSH:
  {
    //IF (![vdvProj,PROJ_POW_OFF])
		IF ([vdvProj,PROJ_POWER_FB]) //CHANGE THE CODE!!!
    {
...
*)
			...
			PULSE[vdvProj,Proj_OFF_IN]		//CHANGE THE CODE!!!
			PULSE[vdvProj2,PROJ_OFF_IN] 	//CHANGE THE CODE!!!
			
			
				
      WAIT_UNTIL([vdvProj,PROJ_COOLING_FB])
      {
				SEND_COMMAND vdvTP,"'TEXT2-PROJECTOR IS COOLING DOWN'"
        SEND_COMMAND vdvTP,"'PPON-PROJ WAIT'"
        //WAIT_UNTIL ([vdvProj,PROJ_POW_OFF]) 'PROJ COOL'
				WAIT_UNTIL (![vdvProj,PROJ_COOLING_FB]) 'PROJ COOL'//CHANGE THE CODE!!!
        {
...
BUTTON_EVENT[dcSource] (* GET_LAST = 1 - 15 *)
{
  PUSH:
  {
    ...
    IF ((![vdvProj,PROJ_POWER_FB] && (BUTTON.INPUT.CHANNEL <> 14) && (BUTTON.INPUT.CHANNEL <> 15))) // IF PROJ IS OFF AND SOURCE IS NOT CASSETTE OR TURNTABLE
    {
      //SEND_COMMAND vdvProj,'PON'
			PULSE[vdvProj,Proj_ON_IN]		//CHANGE THE CODE!!!
      
			TIMELINE_CREATE(TL_WARMING,l_TL_WARMING_ARRAY,1,TIMELINE_ABSOLUTE,TIMELINE_ONCE)
      ...
      WAIT_UNTIL ([vdvProj,PROJ_POWER_FB]=1) 'PROJ WARM'   // WAIT UNTIL PROJECTOR IS ON - BUT NOT LONGER THAN 48 SECS <-WHERE IS THIS SET??
      {
    ...
    IF ((![vdvProj2,PROJ_POWER_FB] && (BUTTON.INPUT.CHANNEL <> 14) && (SystemName[SYSTEM_NUMBER] = 'PAC 002') && (BUTTON.INPUT.CHANNEL <> 15))) // IF PROJ2 IS OFF AND SOURCE IS NOT CASSETTE OR TURNTABLE
    {
      SEND_COMMAND vdvProj2,'PON'
			PULSE[vdvProj2,Proj_ON_IN]	//CHANGE THE CODE!!!
      SYSTEM_CALL[3] 'SCREEN2' (0,0,1,0,dvRELAY,6,5,0,0)     // SCREEN2 IS DOWN
      WAIT_UNTIL ([vdvProj2,Proj_ON_IN]=1) 'PROJ2 WAIT'
      {
...
BUTTON_EVENT[vdvTP,27]                          // PROJ1 POWER ON
{
  PUSH:
  {
    PULSE[vdvProj,PROJ_ON_IN]
    SEND_COMMAND vdvTP,"'PPON-PROJ WAIT'"
    
    CANCEL_WAIT_UNTIL 'PROJ COOL'
    
    WAIT_UNTIL ([vdvProj,PROJ_POWER_FB]=1) 'PROJ WARM'
    {
      SEND_COMMAND vdvTP,"'PPOF-PROJ WAIT'"
    }
  }
...
BUTTON_EVENT[vdvTP,28]                          // PROJ1 POWER OFF
{
  PUSH:
  {
    ...
    //WAIT_UNTIL ([vdvProj,PROJ_POW_OFF]) 'PROJ COOL'
		WAIT_UNTIL (![vdvProj,PROJ_POWER_FB]) 'PROJ COOL'//CHANGE THE CODE!!!
    {
      SEND_COMMAND vdvTP,"'PPOF-PROJ WAIT'"
    }
  }
...
BUTTON_EVENT[vdvTP,120]                         // PROJ1 TOGGLE MUTE
{
  PUSH:
  {
	//    [vdvProj,PROJ_MUTE] = ![vdvProj,PROJ_MUTE]
		IF([vdvProj,PROJ_MUTE_FB])				//CHANGE THE CODE!!!
			PULSE[vdvProj,Proj_MUTEOFF_IN] 	//CHANGE THE CODE!!!
		ELSE
		{//CHANGE THE CODE!!!
			ON[BLINK]
		    PULSE[vdvProj,Proj_MUTEON_IN]		//CHANGE THE CODE!!!
		}
		
		IF([vdvProj2,PROJ_MUTE_FB])  					//CHANGE THE CODE!!!
			PULSE[vdvProj2,Proj_MUTEOFF_IN] 	//CHANGE THE CODE!!!
		ELSE
		{//CHANGE THE CODE!!!
			ON[BLINK]
			PULSE[vdvProj2,Proj_MUTEON_IN]		//CHANGE THE CODE!!!
		}
  }
}
...
BUTTON_EVENT[vdvTP,31]                          // PROJ2 POWER ON
{
  PUSH:
  {
    ...
    WAIT_UNTIL ([vdvProj2,Proj_ON_IN]=1) 'PROJ WARM'
    {
      SEND_COMMAND vdvTP,"'PPOF-PROJ WAIT'"
    }
  }
...
BUTTON_EVENT[vdvTP,32]                          // PROJ2 POWER OFF
{
  PUSH:
  {
    ...
    //WAIT_UNTIL ([vdvProj2,PROJ_POW_OFF]) 'PROJ COOL'
		WAIT_UNTIL (![vdvProj2,PROJ_ON_IN]) 'PROJ COOL'//CHANGE THE CODE!!!
    {
      SEND_COMMAND vdvTP,"'PPOF-PROJ WAIT'"
    }
  }
...
{
	IF(![vdvProj,PROJ_POWER_FB])
	{
		SEND_COMMAND vdvTP,"'TEXT2-PROJECTOR HAS FAILED TO TURN ON'"
		CANCEL_WAIT_UNTIL 'PROJ WARM'
		
		SmtpQueMessage('amx@wesleyan.edu',EmailToWho,
								 "'AMX Alert - ',SystemName[SYSTEM_NUMBER]",
								 "'The Projector has failed to turn on as requested by user.',13,10,
								 'Room: ',SystemName[SYSTEM_NUMBER],13,10,
								 'Time: ',TIME,13,10",'')
		WAIT 70
		{
...
[vdvTP,27] = [vdvProj,PROJ_POWER_FB]                  // FEEDBACK FOR PROJ1 PWR ON
//[vdvTP,55] = [vdvProj,PROJ_POW_OFF]                 // FEEDBACK FOR PROJ1 PWR OFF
[vdvTP,55] = ![vdvProj,PROJ_POWER_FB]//CHANGE THE CODE!!!                 // FEEDBACK FOR PROJ1 PWR OFF
[vdvTP,29] = [vdvProj,PROJ_MUTE_FB]                    // FEEDBACK FOR PROJ1 MUTE ON
[vdvTP,30] = ![vdvProj,PROJ_MUTE_FB]                   // FEEDBACK FOR PROJ1 MUTE OFF
[vdvTP,120] = ([vdvProj,PROJ_MUTE_FB] AND BLINK)       // FEEDBACK FOR PROJ 1 TOGGLE MUTE
[vdvTP,31] = [vdvProj2,PROJ_POWER_FB]                   // FEEDBACK FOR PROJ2 PWR ON
//[vdvTP,56] = [vdvProj2,PROJ_POW_OFF]                  // FEEDBACK FOR PROJ2 PWR OFF
[vdvTP,56] = ![vdvProj2,PROJ_POWER_FB]//CHANGE THE CODE!!!                 // FEEDBACK FOR PROJ1 PWR OFF

[vdvTP,33] = [vdvProj2,PROJ_MUTE_FB]                    // FEEDBACK FOR PROJ2 MUTE ON
[vdvTP,34] = ![vdvProj2,PROJ_MUTE_FB]                   // FEEDBACK FOR PROJ2 MUTE OFF
